...
if grep flutter pubspec.yaml > /dev/null; then
  ...
  if [ ! -d "$MELOS_ROOT_PATH/coverage_html" ]; then
    mkdir "$MELOS_ROOT_PATH/coverage_html"
  fi

  genhtml "$MELOS_ROOT_PATH/coverage_report/lcov.info" -o "$MELOS_ROOT_PATH/coverage_html"
fi
